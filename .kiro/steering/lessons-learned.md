...
63. **useFocusTrap statt manueller Escape-/Fokus-Logik**: Jeder Dialog hatte seine eigene Ad-hoc-Implementierung (document-level keydown, manuelle previousFocus-Refs). Das f�hrt zu inkonsistentem Verhalten (manche geben Fokus zur�ck, manche nicht; Tab kann aus manchen Dialogen entkommen). Der zentrale Hook in `hooks/useFocusTrap.ts` normalisiert: Tab-Zirkulation, Escape-Callback, Fokus-R�ckgabe � einheitlich f�r alle 6+ Modals. Neue Dialoge M�SSEN den Hook nutzen.
64. **`eslint-plugin-jsx-a11y` unter ESLint 10 hei�t `eslint-plugin-jsx-a11y-x`**: Das Original hat `peerDependency: eslint ^3�^9`. Der Fork von `es-tooling` (`-x`-Suffix) unterst�tzt `^9 || ^10`, gleiche Regeln, gleiche API (`configs.recommended`), anderer Package-Name. Namespace in Config: `jsx-a11y-x/rule-name`.
65. **Farbkontrast-Token-Kaskade**: Wenn ein Token ge�ndert wird (z.B. `--text-muted`), MUSS auch `obsidian-compat.css` gepr�ft werden � die Obsidian-Variablen (`--text-faint`, `--text-accent`, etc.) mappen auf dieselben Werte. Eine �nderung nur in `index.css` l�sst Plugin-UI mit dem alten Wert zur�ck.
66. **`vitest-axe` vs. `jest-axe`**: Das Projekt nutzt Vitest, nicht Jest. `vitest-axe` (`1.0.0-pre.5`, MIT) ist der korrekte Wrapper � er registriert `toHaveNoViolations()` �ber `vitest-axe/extend-expect` in `test-setup.ts`. `jest-axe` w�rde nur mit dem Jest-Globals-Setup funktionieren. axe-Core ist transitiv (`axe-core@4.x`).
67. **`aria-hidden="true"` auf rein-dekorativen/redundanten Widgets**: Die Canvas-Minimap ist eine visuelle �bersichtskarte ohne eigene Tastatur-Aktionen � `aria-hidden` verhindert, dass Screenreader sie durchlaufen. Gleiches Prinzip f�r dekorative Icons (`aria-hidden="true"`auf Lucide-Icons NEBEN Text).
68. **Resize-Handles (`role="separator"`) brauchen ALLE vier aria-value-Attribute**: `aria-valuenow`, `aria-valuemin`, `aria-valuemax`, `aria-orientation`. Ohne `valuenow` k�ndigt NVDA nur "Trennlinie" an, ohne den aktuellen Wert. Der Hook `useResize` liefert `min`/`max` jetzt mit, sodass das Handle sie setzen kann.
69. **Akzeptierte Abweichungen dokumentieren, nicht still ignorieren**: Border-Tokens (`--border-subtle`, `--border-default`) fallen ALLE unter die 3:1-UI-Schwelle, sind aber bewusst so (dekorativ, nicht einziges Erkennungsmerkmal). Ohne dokumentierte Begr�ndung (`ACCESSIBILITY-AUDIT.md`, Abschnitt "Akzeptierte Abweichungen") sieht das beim n�chsten Audit wie ein offener Bug aus.