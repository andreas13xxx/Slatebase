...
│   ├── regex-safety.ts       — MAX_REGEX_LENGTH + validateRegexPattern() (length check, syntax check, nested-quantifier ReDoS heuristic) — throws RegexTooLongError/RegexValidationError, shared by search and replace
│   ├── regex-safety.test.ts  — Unit tests for regex-safety
...
│   ├── secret-key-manager.ts — PluginSecretKeyManager (encryption key for plugin secrets at rest: env `SLATEBASE_PLUGIN_SECRET_KEY` → file → generate, same pattern as CsrfSecretManager; HKDF-SHA256 down to an AES-256 key)
│   ├── secret-store.ts   — PluginSecretStore (`data/plugins/<vaultId>/<pluginId>/secrets.json`, each value encrypted individually with AES-256-GCM; `listSecrets()` returns IDs only, never values)
...
│   ├── CodeMirrorEditor.tsx  — React wrapper (EditorView in useRef, props→effects sync, mode toggle); marks the wrapper's parent `.markdown-source-view` and publishes its grandparent as containerEl for plugin toolbars; `readableLineLength` toggles the `cm-full-width` class, `spellcheck`/`spellcheckLanguage` drive the spellcheck Compartment (the DOM `spellcheck` attribute is always `"false"` — Slatebase checks spelling itself). `commitFocusedWidgetEdit()` blurs a focused widget-hosted input while the view is still alive (before `saveCurrentState()`/`destroy()`, on both the tab-switch and unmount paths), since the frontmatter property inputs commit on blur and a destroyed view fires none. `openContextMenuAt()` is shared by the mouse handler and the keyboard-triggered `showContextMenu()`: it looks the misspelled word up in the lint diagnostics, opens the menu immediately, then swaps in the worker's suggestions when they arrive
...
│       ├── async-lifecycle.ts — Containment for lifecycle hooks a plugin declared `async`. Obsidian's `load()`/`unload()` return `void`, so every caller drops the promise an `async onload()`/`onunload()` returns — a rejection in the *asynchronous* part of a teardown then surfaces as a bare "Uncaught (in promise)" with no plugin id, and during a vault switch lands after the next vault's plugin system is already being built. Every lifecycle call site (`install-globals.ts`, `fallback-shims.ts`, `plugin-loader.ts`) hands its return value here: non-thenables pass through untouched, a thenable gets an immediate rejection handler, and an unload's promise is parked on the instance in a `WeakMap` for `takePendingUnload()`
│       ├── plugin-loader.ts — PluginLoader (bundle evaluation, lifecycle, timeout, cleanup, @lezer/* stubs). `deactivatePlugin()` awaits a parked `async onunload()` before `sandbox.cleanup()` — bounded to 3s, because a hung teardown must not block a vault switch
...
│       ├── ribbon-icon-registry.ts — Module-level ribbon icon registry (addRibbonIcon store + change listeners). Rendered by `PluginRibbonIcon`, whose button also carries Obsidian's `side-dock-ribbon-action` class so plugin CSS matches; the click callback forwards the real `MouseEvent` (not called with no arguments) since plugins read `evt.shiftKey`/`ctrlKey` (Excalidraw)
...
│       ├── body-classes.ts — Syncs Obsidian's `theme-dark`/`theme-light` + platform marker classes (`is-mobile`, `mod-macos`, …) onto `document.body`; plugin CSS and runtime theme checks read those, while Slatebase's own state lives in `data-theme` on `<html>`. Exports `OBSIDIAN_HOST_BODY_CLASSES` as the single source of truth, which `css-injector.ts` imports so the setting and the scoping side cannot drift
...
│       ├── notice.test.ts — Unit tests for the `Notice` shim: real container/notice/message tree, DocumentFragment kept as markup instead of flattened, and the live `noticeEl` — not a copy — handed to the toast bridge
│       ├── deliberate-noops.test.ts — Pins down that every intentionally unimplemented stub logs (CM5 legacy, Vim adapter, foldManager, onNodeInserted/onWindowMigrated, bootstrap `app.commands`) and at the right severity — debug for a trade-off with a working alternative, warn where plugin code silently will not run
│       ├── editor-shim.gap.test.ts — Unit tests for `EditorShim.create()`'s Proxy net: emulated methods pass through with `this` intact, unknown ones answer with a logged no-op recorded in the gap registry, and `then` stays undefined so an awaited editor cannot hang
│       └── shims/
...
│           ├── workspace-shim.ts — WorkspaceShim (full Leaf API + getActiveViewOfType synthetic view for the MarkdownView/FileView/ItemView family, onLayoutReady error-isolation); every leaf `getLeaf()` hands back carries the built-in "empty"-type view instead of null (plugins call `leaf.view.getViewType()` right after `getLeaf()`, before `setViewState()` attaches a real one), and the reuse branch looks for that empty view rather than a null one, and layout/leaf events re-fire once the CM6 editor mounts. `iterateLeaves()`/`onLayoutChange()` legacy aliases; `hoverLinkSources` getter exposes the `hover-link-bus.ts` registry
...
│           ├── file-manager-shim.ts — FileManagerShim (renameFile, processFrontMatter, generateMarkdownLink, getNewFileParent, createNewFolder, createAndOpenMarkdownFile, trashFile, promptForFileRename, promptForFileDeletion, promptForFolderDeletion, getAvailablePathForAttachment)
...
│   ├── tabState.ts       — Tab reducer + types; `TabEntry.pinned` (survives `CLOSE_OTHER_TABS`/`TOGGLE_PIN`) and `TabState.closedTabsHistory` (capped `MAX_CLOSED_TABS_HISTORY`=20, `POP_CLOSED_TAB`) back `workspace:toggle-pin`/`workspace:undo-close-pane`
...
│   ├── settingsLabels.ts     — SECTION_LABELS/CATEGORY_LABELS (single source for sidebar nav + content heading, was duplicated — and had drifted — across SettingsNavList.tsx/SettingsContent.tsx)
...
├── i18n/
│   ├── index.ts          — I18nProvider + useTranslation hook: locale from user profile (fallback to browser detection pre-login), dot-path key resolution against the DE/EN objects with German fallback on missing keys, `{param}` interpolation
...
│   ├── useDropZone.ts    — External-file drag-and-drop hook (drag counter, size/count validation, toast errors). Only reacts to drags carrying OS files (`dataTransfer.types` contains `'Files'`) — internal drags (explorer node, tab, toolbar button) carry custom MIME types only and must not light up the drop overlay. A capture-phase `drop`/`dragend` listener on `window` clears a stuck highlight when the drop never reaches this hook's handler (an inner handler stopped propagation, or the user released outside the zone)
...
│   ├── TabBar.tsx        — Unified horizontal tab strip: settings-page tabs (not draggable) + file tabs (draggable/reorderable) in one row; pinned tabs show a persistent pin icon instead of a close button (`workspace:toggle-pin`), pin/unpin also in `tab-context-menu.ts`
│   ├── InfoModal.tsx     — Shared overlay/focus-trapped-panel/header/close-button/scrollable-body chrome for simple read-only info dialogs, so Debug Info and Release Notes don't duplicate the same markup (modeled on PluginDetailPanel's overlay/panel structure)
│   ├── InfoModal.css     — InfoModal styles
│   ├── DebugInfoModal.tsx — `app:show-debug-info`: version/browser/platform/active-vault dump with copy-to-clipboard, in an InfoModal
│   ├── DebugInfoModal.test.tsx — Unit tests for DebugInfoModal
│   ├── ReleaseNotesModal.tsx — `app:show-release-notes`: last 5 GitHub releases (via useReleaseNotes), bodies rendered through simpleMarkdownToHtml, in an InfoModal
│   ├── ReleaseNotesModal.test.tsx — Unit tests for ReleaseNotesModal
...
│   ├── EditMode.tsx      — CodeMirror editor host: auto-save (1.5s debounce, flushed rather than cancelled on unmount, on every `tabId` change, and on `pagehide`/`visibilitychange` — the component stays mounted across tabs, so a pending timer would otherwise fire against the *new* tab's `onSave`) + undo/redo + line numbers + image paste + DnD + read-only banner + editor command event listener (slatebase:editor-command). No native formatting toolbar — formatting runs through the Command Palette or an Obsidian-compatible plugin toolbar (Editing Toolbar); `livePreviewMode` is a required prop driven by the tab mode
...
│   │   ├── PropertiesEditor.tsx  — Interactive typed frontmatter editor: type resolution (registry > well-known keys > inference), typed controls per property, add/delete/commit. The inline key rename commits on blur *and* on unmount (`useCommitOnUnmount`), for the same reason as the value controls
│   │   ├── PropertiesEditor.css
│   │   ├── property-controls/   — Individual type-aware input controls for the Properties Editor
│   │   │   ├── index.ts          — Barrel export
│   │   │   ├── useCommitOnUnmount.ts — Commits a still-open inline edit when a control unmounts. React fires no blur on unmount, and unmounting mid-edit is routine here: every commit rewrites the frontmatter block, which tears down and rebuilds the whole properties editor (FrontmatterWidget), as does toggling Live Preview. Reads `flush` from a ref (so the cleanup commits the *latest* draft) and defers by a microtask (unmount can happen inside a CM6 update or React commit, where dispatching is re-entrant). Callers guard `flush` with a settled-*ref* — not state, which would not have re-rendered before the unmount
│   │   │   ├── TextPropertyControl.tsx — Click-to-edit text input (Enter/Blur commit, Escape cancel, plus `useCommitOnUnmount` for the blur that never comes)
│   │   │   ├── NumberPropertyControl.tsx — Numeric input with parseFloat validation; same draft/commit lifecycle as the text control
│   │   │   ├── DatePropertyControl.tsx — Native date/datetime-local picker; local-draft-until-blur/Enter like the text/number controls, because a native date input fires `change` once per completed segment (year, month, day) and a commit remounts the whole properties editor — committing on every `change` would destroy the field mid-entry after the first segment; the draft is likewise flushed via `useCommitOnUnmount`
...
│   │   ├── SettingsPanel.css     — Settings panel styles (responsive layout, embedded table overrides); also sets `min-width:0` on `.settings-panel-content` — it's a row-flex item once the container query goes wide, and without this a wide tab (a table) grows the flex item past the modal instead of scrolling inside it
...
│   │   ├── SettingsNavList.tsx   — Category/section nav (keyboard nav, aria-current, disabled vault sections); labels from `state/settingsLabels.ts`
│   │   ├── SettingsNavList.css   — Nav list styles
│   │   ├── SettingsContent.tsx   — Section → Component mapping with focus management; heading from `state/settingsLabels.ts`
...
└── <vaultId>/
    ...
    └── <pluginId>/
        ...
        ├── main.js           — Plugin bundle (JavaScript, max 10 MB; ZIP upload capped at 10 MB, max 20 MB extracted)
        ├── styles.css        — Plugin styles (optional, max 512 KB)
        ├── data.json         — Plugin settings (max 1 MB, preserved across upgrades)
        └── secrets.json      — Plugin secrets (SecretStorage API), each value AES-256-GCM encrypted; key from `SLATEBASE_PLUGIN_SECRET_KEY` or `data/.plugin-secret-key`
```
