...
│           ├── workspace-shim.ts — WorkspaceShim (full Leaf API + getActiveViewOfType synthetic view for the MarkdownView/FileView/ItemView family, onLayoutReady error-isolation); active leaf falls back to an "empty"-type view instead of null, and layout/leaf events re-fire once the CM6 editor mounts. `iterateLeaves()`/`onLayoutChange()` legacy aliases; `hoverLinkSources` getter exposes the `hover-link-bus.ts` registry
