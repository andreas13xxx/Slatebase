...
- Vault sync ⚠️ experimental (CouchDB/livesync, bidirectional, conflict resolution wizard with 3-step flow, auto-resolution strategies, batch operations, inline diff view, E2E encryption; **also available via Obsidian LiveSync plugin** running natively in the plugin compat layer with server-side CORS proxy)
...
- Obsidian plugin compat ⚠️ experimental (API shims, sandbox, command palette, CSS injection, workspace leaf API with plugin views as tabs/sidebar sections; extended API coverage for Calendar, Dataview, Templater, LiveSync, Excalidraw, Editing Toolbar, Kanban — full EditorShim with CM6 backend, MarkdownView stub, editorCallback commands, MarkdownRenderer.render(), Notice→Toast bridge, requestUrl() backend proxy, registerExtensions file routing, CodeBlockProcessor registry with ViewMode integration, SuggestModal/FuzzySuggestModal, getAllTags, debounce, App.loadLocalStorage/saveLocalStorage, Vault.copy, Obsidian CSS variables, DOM extensions, icon registry; **LiveSync full compat**: CORS proxy for cross-origin fetch/XHR, PouchDB IndexedDB local DB, bidirectional CouchDB replication via server-side proxy, vault event bridge for push sync, binary file upload support, plugin view tabs)
