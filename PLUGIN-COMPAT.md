...
| `Plugin` | ✅ Vollständig | Lifecycle, `addCommand`/`addSettingTab`/`registerView`/`registerEditorExtension`/`loadData`/`saveData` real; `registerBasesView`/`registerObsidianProtocolHandler`/`registerCliHandler` bewusste No-Ops; `registerEditorSuggest` bridged nur CM6-`provider`, nicht Obsidians natives Interface. |
...
| `SettingPage` | ⚠️ Stub | Nur DOM-Grundgerüst, `display()`/`hide()` leere No-Ops. |
...
| `EditorSuggest` | ⚠️ Stub | **Kernbefund:** Obsidians natives `onTrigger`/`getSuggestions`/`renderSuggestion`/`selectSuggestion`-Interface wird vom Framework nie aufgerufen — nur ein von Plugins selbst bereitgestellter CM6-`provider` wird gebridged. `setInstructions()` ist ein bewusster kosmetischer No-Op. |
| `MarkdownView` | ✅ Vollständig | Echte Klassenkette für `instanceof` (Templater-getestet). Erbt entgegen der realen API nicht von `TextFileView` — `markdownView instanceof TextFileView` ist `false` statt `true`. |
| `MarkdownEditView` | ❌ Fehlt | Nicht registriert, `require('obsidian').MarkdownEditView` liefert `undefined`. |
| `MarkdownPreviewView` | ⚠️ Stub | `render()`, `getScroll()`/`applyScroll()` (auf `contentEl.scrollTop`) und `get()`/`set()` (internes Datenfeld) implementiert; `rerender()` ist ein No-Op. |
| `MarkdownRenderer` | ✅ Vollständig | Echte unified/remark-Pipeline; Wikilinks/Callouts/Embeds nicht erkannt. |
| `MarkdownRenderChild` | ✅ Vollständig | Erbt vollen Component-Lifecycle. |
| `MarkdownPreviewRenderer` | ⚠️ Stub | Schreibt in eine isolierte, nie gelesene Registry — der wirksame Post-Processor-Pfad läuft komplett getrennt über `Plugin.registerMarkdownPostProcessor()`. |
...
| `EditableFileView` | ⚠️ Stub | Existiert nicht eigenständig, nur nachträglicher Alias auf `FileView`. |
...
| `Notice` | ⚠️ Stub | `duration`/`timeout` steuert den Auto-Dismiss (`duration: 0` unterdrückt ihn wie in echtem Obsidian); `hide()`/`setMessage()` adressieren den tatsächlich angezeigten Toast. `containerEl`/`messageEl`/`noticeEl` bleiben bewusst DOM-losgelöste Platzhalter — Plugin-Manipulationen daran erscheinen nicht im gerenderten Toast. |
...
- **`instanceof`-Ketten** (`MarkdownView`/`TextFileView`/`EditableFileView`): Korrekt beheben hieße, mehrere zentrale, stark getestete Klassenhierarchien umzubauen — Regressionsrisiko für praktisch jedes View-lastige Plugin, nicht punktuell eingrenzbar. (`WorkspaceLeaf`s eigene `instanceof`-Lücke wurde behoben — s. o. — weil dort die reale Leaf-Klasse bereits existierte und nur unter dem falschen globalen Namen registriert war, kein Umbau einer Klassenhierarchie nötig war.)
- **`AbstractInputSuggest`/`EditorSuggest`/`PopoverSuggest` — echtes Dropdown-Popover**: Ein funktionierendes Such-Popover mit Tastatur-Navigation ist ein eigenständiges Feature, kein kleiner Fix (vergleichbar mit `SuggestModal`, aber ohne dessen Modal-Kontext).
