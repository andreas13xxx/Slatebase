...
|---|---:|---:|
| Voll kompatibel | 51 | 51 % |
| Teilweise kompatibel | 38 | 38 % |
| Nicht unterstützt (Desktop-only) | 11 | 11 % |

12 der 100 Plugins wurden zusätzlich manuell mit ihrem echten, aus GitHub geladenen Bundle in der laufenden App getestet (nicht nur statisch analysiert) — siehe [Manuell getestete Plugins](#manuell-getestete-plugins).
...
`workspace.getLeavesOfType('file-explorer')` liefert ein synthetisches Leaf mit `view.fileItems: Record<path, { file, titleEl, selfEl }>` — ein echtes DOM-Element pro aktuell gerenderter Datei-/Ordner-Zeile, das Plugins wie **Iconize** (`obsidian-icon-folder`) über `fileItems[path]` → `titleEl.querySelector('.iconize-icon')` finden und selbst mit einem eigenen Icon-Element befüllen. `titleEl` ist bewusst der Zeilen-Button selbst (nicht ein eigens dafür gebautes Wrapper-Element): seine React-Kinder (Icon, Name, Favoriten-Stern) haben eine über Re-Renders hinweg stabile Anzahl/Reihenfolge, sodass React nur die ihm bekannten Kind-Fibers gezielt aktualisiert und ein vom Plugin zusätzlich angehängtes Element unangetastet lässt.

**Zweiter, unabhängiger DOM-Pfad (nicht über `fileItems`):** Statische Analyse von Iconize v2.14.7 (`backend/data/plugins/*/obsidian-icon-folder/main.js`, echte Installation) zeigt, dass die interaktive Icon-Zuweisung (Icon-Picker, Rechtsklick „Change icon"/„Change color") gar nicht über `fileItems` läuft, sondern über `dom.createIconNode()`/`dom.getIconNodeFromPath()`, die direkt am Dokument scrapen:
```js
const node = document.querySelector(`[data-path="${path}"]`);
let titleNode = node.querySelector('.nav-folder-title-content') ?? node.querySelector('.nav-file-title-content');
if (!titleNode) { logger.warn(...); return; }  // stiller Abbruch, kein Icon eingefügt
let iconNode = node.querySelector('.iconize-icon') ?? /* neu erzeugt, Klasse .iconize-icon, Attribut data-icon */;
node.insertBefore(iconNode, titleNode);  // wirft, wenn titleNode kein direktes Kind von node ist
// ...
// Für die Farbänderung:
document.querySelector(`[data-path="${path}"]`)?.querySelector('[data-icon]')
```
Damit das funktioniert, muss **dasselbe** Element sowohl `data-path` tragen als auch `.nav-folder-title-content`/`.nav-file-title-content` als **direktes** Kind enthalten (`insertBefore` verlangt eine echte Eltern-Kind-Beziehung) — exakt wie in echtem Obsidian, wo `.nav-folder-title`/`.nav-file-title` (der `data-path`-Träger) und `.nav-folder-title-content`/`.nav-file-title-content` (Text-Span) Eltern und direktes Kind sind. Slatebases Zeilen-`<button>` (`tree-node-toggle`/`tree-node-file`) trägt deshalb jetzt zusätzlich `data-path` sowie die Klassen `nav-folder-title`/`nav-file-title`, und der Name-Span zusätzlich `nav-folder-title-content`/`nav-file-title-content` — rein additiv neben den bestehenden Slatebase-Klassen. Das äußere `<li>` (das früher `data-path` trug) nutzt jetzt das interne, Slatebase-eigene `data-node-path`, um exakt eine Fundstelle pro Pfad im DOM zu garantieren; ein bloßes `document.querySelector('[data-path="…"]')` muss immer eindeutig auf die Titel-Zeile treffen, nicht auf einen Vorfahren.

**Nachtrag (behoben statt offen gelassen):** Der ursprünglich hier als „bewusst nicht verfolgt" eingestufte Reload-Fall stellte sich als der eigentliche Haupt-Pfad heraus, nicht als Randfall: Iconizes `addAll()` — aufgerufen bei **jedem** Plugin-Load, also bei jedem Seiten-Reload und jedem Vault-Wechsel — durchläuft exakt diesen `fileItems[path]` + `titleEl.children.length === 1 || 2`-Guard, um zuvor gespeicherte Icons erneut anzuwenden. Ohne diesen Guard zu erfüllen, verschwand ein zugewiesenes Icon nach jedem Reload wieder, und die anschließende Farbänderung crashte erneut mit `iconNode is null` — derselbe Fehler wie beim allerersten Zuweisen, nur diesmal nach dem Neuladen ausgelöst.

Behoben durch Umstrukturierung der Zeilen-DOM in `TreeNode.tsx`: Icon + Name (+ Favoriten-Stern bei Dateien) stecken jetzt in einem gemeinsamen `<span class="tree-node-name-wrapper nav-folder-title-content">` bzw. `nav-file-title-content` — dadurch hat der Titel-Button im Ruhezustand exakt die von Iconize erwartete Kinderzahl:
- Datei-Button: **1** Kind (der Wrapper-Span; vorher 3: Icon, Name, Stern als Geschwister).
- Ordner-Button: **2** Kinder (Chevron + Wrapper-Span; vorher 3: Chevron, Ordner-Icon, Name als Geschwister).

Das entspricht exakt echtem Obsidians Ausgangszustand (`nav-file-title` = nur `nav-file-title-content`; `nav-folder-title` = `collapse-icon` + `nav-folder-title-content`). Iconizes eigenes Icon landet dabei als zusätzliches Geschwister-Element vor dem Wrapper (weiterhin zusätzlich zum Slatebase-eigenen Icon sichtbar, nicht als dessen Ersatz — siehe CSS-Scoping-Einschränkung oben). Reine additive/strukturelle Änderung, keine Verhaltensänderung der bestehenden Zeilen-Interaktion (Klick, Drag&Drop, Rename, Favoriten). Abgesichert durch drei neue Tests in `FileExplorer.test.tsx` (`describe('Iconize-compatible DOM shape …')`), die die Kinderzahl und die `data-path`-Eindeutigkeit direkt prüfen.

Drei Abweichungen von echtem Obsidian, bewusst in Kauf genommen:
...
`view.containerEl` ist bewusst ein statischer, nie ins DOM eingehängter Platzhalter statt des echten, dauerhaften `.file-explorer-tree`-Elements — ein Plugin- oder Framework-Aufruf von `containerEl.remove()` (z. B. via `leaf.detach()`) dürfte niemals die echte, geteilte Explorer-UI aus der Seite reißen. Alle übrigen `view`-Properties (`sortOrder`, `createFolderDom`, `requestSort`, …) sind über dasselbe Proxy-Sicherheitsnetz wie `WorkspaceLeaf` abgefangen (No-Op + einmalige Warnung, sichtbar über `window.__slatebasePluginApiGaps()`).

Aus demselben Grund ist das Leaf **nicht** in der `ViewRegistry` getrackt (kein `viewRegistry.createLeaf()`): `ViewRegistry.clear()` — bei jedem Vault-Wechsel aufgerufen — würde sonst `view.containerEl.remove()` auf jedes getrackte Leaf anwenden.

**Nicht Teil dieses Schritts:** Iconize-eigenes CSS (z. B. zum Ausblenden des Standard-Datei-Icons) greift im Explorer nicht automatisch — der `CssInjector` scoped Plugin-Styles auf `[data-plugin-id="…"]`-Subtrees, die der Datei-Explorer nicht trägt. Ein injiziertes Icon-Element erscheint also zusätzlich zum Slatebase-eigenen Icon, nicht als dessen Ersatz.
...
- **`fileManager.promptForFolderDeletion`**, `fileManager.createNewFolder` und `fileManager.createAndOpenMarkdownFile`: nicht emuliert (Alternativen: `vault.createFolder`, `createNewMarkdownFile` + manuelles Öffnen). Der Datei-Löschdialog (`fileManager.promptForFileDeletion()`, der echte Obsidian-API-Name) ist implementiert (`file-manager-shim.ts`) und zählt als grün.
...
- **`vault.getConfig('updateInternalLinks')`**: nicht als bekannter Config-Key emuliert (liefert `null` statt `true`/`false`) — das eigentliche Verhalten dahinter (Wikilinks vaultweit umschreiben bei Umbenennen/Verschieben) läuft seit der Link-Migration (`.kiro/specs/graph-polish-link-integrity/`) serverseitig immer, es gibt nur keinen abfragbaren Ein/Aus-Schalter dafür.
...
| `Workspace` | ✅ Vollständig | Leaf-API/Events/Tabs real; Split-Baum (`getLayout`/`rootSplit`/Ribbons) sind dokumentierte Stubs, jetzt mit korrekter `instanceof`-Identität (`WorkspaceSplit`/`WorkspaceRibbon`, s. u.). Mehrere Workspace-Events (`file-menu`, `editor-menu`, `css-change`, `quit`, …) sind registrierbar, werden aber nirgends im Code ausgelöst; `activeLeaf instanceof WorkspaceLeaf` funktioniert (s. u.). |
...
| `FileManager` | ✅ Vollständig | `renameFile()` aktualisiert seit der Link-Migration (`.kiro/specs/graph-polish-link-integrity/`) automatisch alle Wikilinks anderer Dateien, die auf den alten Pfad zeigen — läuft serverseitig im selben `moveContent`/`renameContent`-Endpunkt, den `vault.rename()` aufruft, unabhängig vom Auslöser (UI, Plugin, REST). `processFrontMatter()` refresht den MetadataCache nicht mit; `generateMarkdownLink()` ignoriert `sourcePath` (immer absoluter Pfad). |
...
| `Setting` | ✅ Vollständig | Fluent-Builder mit allen `add*`-Methoden real, inkl. `setVisibility()`, `errorEl`/`setErrorMessage()` (seit 1.13.0) und `setTooltip()` (echte aria-label-Mechanik). |
...
| `Events` | ✅ Vollständig | `on`/`off`/`trigger` real; `offref()`/`tryTrigger()` No-Ops. |
...
| `Editor` | ✅ Vollständig | Fast komplettes CodeMirror-Interface (~40 Methoden geprüft); `exec()` deckt 14/17 `EditorCommandName` ab (kein Code-Folding), `undo`/`redo` ohne CM6 wirkungslos. |
| `EditorSuggest` | ✅ Vollständig | Obsidians natives `onTrigger`/`getSuggestions`/`renderSuggestion`/`selectSuggestion`-Interface wird über einen CM6-ViewPlugin bei jeder Cursor-Bewegung getriggert. Positioniertes Dropdown-Popover mit Keyboard-Navigation (↑↓ Enter Tab Esc). `setInstructions()` ist ein bewusster kosmetischer No-Op (Footer-Hints werden nicht angezeigt). Async-`getSuggestions` via Generation-Guard stale-sicher. |
...
| `MarkdownPreviewRenderer` | ✅ Vollständig | `registerPostProcessor()` delegiert jetzt an die echte `code-block-processor-registry` (mit Plugin-ID aus dem Ausführungskontext). Registrierte Post-Processors werden tatsächlich beim Rendering ausgeführt. |
...
| `WorkspaceLeaf` | ✅ Vollständig | `window.obsidian.WorkspaceLeaf` ist jetzt dieselbe Klasse wie die echten Leaf-Objekte aus `view-registry.ts` (nicht mehr ein separater Stub) — `instanceof`-Check funktioniert für jedes reale Leaf (`activeLeaf`, `getLeaf()`, `getRightLeaf()`, …). |
| `WorkspaceSplit`/`WorkspaceRibbon` | ✅ Vollständig, mit Einschränkung | Echte, gemeinsam genutzte Klassen (`view-registry.ts`) für `WorkspaceShim`s `rootSplit`/`leftSplit`/`rightSplit`/`leftRibbon`/`rightRibbon`-Stub-Objekte — `instanceof`-Check funktioniert, aber es gibt keinen echten Split-Baum dahinter (`children` bleibt leer, Collapse/Expand sind No-Ops). |
...
| `AbstractInputSuggest` | ⚠️ Stub | **Kein echtes Dropdown-Popover** — `getSuggestions()`/`renderSuggestion()` werden nie automatisch ausgelöst. |
| `PopoverSuggest` | ⚠️ Stub | `open()`/`close()` leere No-Ops — Basismechanik für alle Suggest-Popover fehlt. |
| `Menu` | ✅ Vollständig | Vollständige Item-/Submenu-Logik inkl. `setParentElement()` und `static forEvent()`. |
| `MenuItem` | ✅ Vollständig | Echte Klasse (`frontend/src/plugins/compat/menu.ts`) — `menu.addItem()`-Einträge sind jetzt `instanceof MenuItem`. |
...
- ~~**`instanceof`-Ketten** (`MarkdownView`/`TextFileView`/`EditableFileView`)~~: **Behoben** (August 2026). `MarkdownView` erbt jetzt korrekt von `TextFileView → EditableFileView → FileView`. `instanceof TextFileView`/`EditableFileView` liefern `true`. `MarkdownEditView` und `MarkdownPreviewView` sind als echte Klassen registriert, `MarkdownRenderer` ist instanziierbar (extends `MarkdownRenderChild`).
...
| 30 | [Self-hosted LiveSync](https://github.com/vrtmrz/obsidian-livesync) (`obsidian-livesync`) | 851.999 | vrtmrz | Nein | Ja | Teilweise kompatibel | Aktiv | ✅ Ja (echtes Bundle) | Plugin referenziert Node-Built-ins ohne Browser-Ersatz (`crypto`) — sicher gestubbt (Warnung, kein Absturz), Badge daher gelb statt grün. |
...
| 35 | [Advanced Canvas](https://github.com/developer-mike/obsidian-advanced-canvas) (`advanced-canvas`) | 724.860 | mika-dev | Nein | Ja | Voll kompatibel | Aktiv | – Nur statische Analyse | Statische Analyse: alle erkannten API-Zugriffe vollständig emuliert. |
...
| 37 | [PDF++](https://github.com/ryotaushio/obsidian-pdf-plus) (`pdf-plus`) | 679.335 | ryotaushio | Nein | Ja | Teilweise kompatibel | Aktiv | – Nur statische Analyse | `workspace.hoverLinkSources` ist nicht emuliert. `workspace.getGroupLeaves` ist nicht emuliert. `metadataCache.onCleanCache` ist nicht emuliert. |
...
| 63 | [Folder notes](https://github.com/lostpaul/obsidian-folder-notes) (`folder-notes`) | 399.920 | lostpaul | Nein | Ja | Teilweise kompatibel | Aktiv | – Nur statische Analyse | `fileManager.promptForFolderDeletion()` (Ordner-Löschdialog) ist nicht emuliert. `fileManager.createNewFolder()` ist nicht emuliert (`vault.createFolder()` als Alternative nutzen). (Vormals zusätzlich `workspace.getActiveFileView()` — seit 2026-08-12 emuliert; der Datei-Löschdialog `promptForFileDeletion()` lief separat unter falschem Namen im Shim und wurde am 2026-08-16 korrigiert, betraf diese Zeile aber nicht, da Folder notes nur den Ordner-Dialog nutzt.) |
...
| 76 | [Quiet Outline](https://github.com/guopenghui/obsidian-quiet-outline) (`obsidian-quiet-outline`) | 316.867 | guopenghui | Nein | Ja | Teilweise kompatibel | Aktiv | – Nur statische Analyse | `vault.getConfig('updateInternalLinks')` ist als Config-Key nicht emuliert (das Verhalten dahinter läuft inzwischen serverseitig immer, s. o.). `fileManager.iterateAllRefs()` (Referenz-Iteration über den ganzen Vault) ist nicht emuliert. |
